output "elb_id" {
  ...
  value       = concat(aws_elb.elb.*.id, [""])[0]
}

output "elb_arn" {
  description = "The ARN of the ELB"
  value       = concat(aws_elb.elb.*.arn, [""])[0]
}

output "elb_name" {
  description = "The name of the ELB"
  value       = concat(aws_elb.elb.*.name, [""])[0]
}

output "elb_dns_name" {
  description = "The DNS name of the ELB"
  value       = concat(aws_elb.elb.*.dns_name, [""])[0]
}
...
output "elb_source_security_group" {
  ...
  value       = concat(aws_elb.elb.*.source_security_group, [""])[0]
}
...
output "elb_source_security_group_id" {
  ...
  value       = concat(aws_elb.elb.*.source_security_group_id, [""])[0]
}

output "elb_zone_id" {
  description = "The canonical hosted zone ID of the ELB (to be used in a Route 53 Alias record)"
  value       = concat(aws_elb.elb.*.zone_id, [""])[0]
}